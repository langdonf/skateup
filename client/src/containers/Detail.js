...
class Detail extends React.Component{
	...
	edit=()=>{
		let toSend = {
			...
			"start": {
				lat: this.state.eventDetails.start.lat,
		...
		let eventId = this.props.match.params.eventId
		Axios
			.post(`${backURL}/api/events/edit/${eventId}`, toSend)
			.then( response => {
				window.location=`/profile`
			})
	};
	...
	handleAttend=()=>{
		...
		Axios
			.post(`${backURL}/api/events/attend/${userId}/${eventId}`)
			.then(response => {})
		this.setState({
			disabled: true,
	...
	handleCity = e => {
		var ths = this;
		Axios
			.post(`https://maps.googleapis.com/maps/api/geocode/json?address=${e.target.value}&key=${APIKey.APIKey}`)
			.then(function(response) {
				console.log(response);
				ths.setState({
    ...
    render(){
        ...
        if (window.localStorage.getItem('userId') === deets.owner){
			deleteButton.push(
				<Button key={0} onClick={this.handleClickOpen} color="primary"> 
					Cancel Event
				</Button>, 
						...
						<Grid container spacing={16}>
							...
							<Grid item xs={4}>
								<Typography variant="h5" gutterBottom>
									In {deets.city} on <br/> {moment(deets.date).format("dddd, MMM Do hh:mm A")}
								</Typography>
